...
export function validateCourseStructure(
  ...
  const roleIssues = validateRolePersonalizations(course.rolePersonalizations, course.generationRequest.targetRoles);
  ...
  const isValid = metrics.overallScore >= validationContext.minQualityThreshold && 
...
function validateModules(modules: AIGeneratedModule[]): ValidationIssue[] {
  ...
  modules.forEach((module, index) => {
    ...
    if (module.contentConfig.hasCodeExample && (!module.codeExample || module.codeExample.length < 20)) {
      issues.push({
        type: 'technical',
        severity: 'medium',
...
function calculateQualityMetrics(_course: AIGeneratedCourse, issues: ValidationIssue[]): QualityMetrics {
  ...
  const structuralCompliance = Math.max(0, 1 - (criticalIssues * 0.3 + highIssues * 0.2 + mediumIssues * 0.1));
  const technicalAccuracy = Math.max(0, 1 - (issues.filter(i => i.type === 'technical').length * 0.15));
...
function generateImprovementSuggestions(
  ...
  if (metrics.roleRelevance < 0.8) {
    suggestions.push('Improve role-specific content and add more targeted examples for each engineering role');
  }
  ...
  const avgModuleConfidence = course.generatedModules.reduce((sum, m) => sum + m.confidence, 0) / course.generatedModules.length;
  if (avgModuleConfidence < 0.8) {
    suggestions.push('Improve module content quality and research depth for higher confidence scores');
  }
...
function assessEducationalValue(content: string): number {
  const educationalIndicators = [
    'learn', 'understand', 'implement', 'practice', 'example',
    'step', 'guide', 'tutorial', 'exercise', 'objective'
  ];
...
function assessTechnicalDepth(content: string): number {
  const technicalTerms = [
    'api', 'configuration', 'integration', 'sdk', 'implementation',
    'monitoring', 'tracking', 'debugging', 'performance', 'optimization'
  ];
...
function validateDifficultyForRole(
  ...
  if (!expected.includes(personalization.difficulty)) {
    issues.push(`Difficulty level "${personalization.difficulty}" may not be appropriate for ${role} role`);
  }
