...
export function analyzeContentPatterns(synthesizedContent: SynthesizedContent): ContentPattern {
  ...
  const actionWords = [...extractActionWords(keyTakeaways), ...extractActionWords(bestPractices)]

  // Extract technical concepts
  const techConcepts = extractTechnicalConcepts([...mainConcepts, ...keyTakeaways])
...
export function generateModuleStructure(
  ...
  const conceptModules = mainConcepts
    .slice(0, targetModuleCount - 1)
    .map((concept, index) => createConceptModule(concept, synthesizedContent, patterns, index))

  // Add a practical/hands-on module if we have space
  if (
    conceptModules.length < targetModuleCount &&
    (codeExamples.length > 0 || useCases.length > 0)
  ) {
  ...
  switch (targetStructure) {
    case 'course':
      if (
        ...
        (typeof generatedContent.title === 'string' && generatedContent.title.length < 10)
      ) {
      ...
      if (
        ...
        (typeof generatedContent.description === 'string' &&
          generatedContent.description.length < 50)
      ) {
      ...
      if (
        !generatedContent.generatedModules ||
        (Array.isArray(generatedContent.generatedModules) &&
          generatedContent.generatedModules.length < 3)
      ) {
    ...
    case 'module':
      if (
        ...
        (Array.isArray(generatedContent.keyTakeaways) && generatedContent.keyTakeaways.length < 2)
      ) {
    ...
    case 'learning_path':
      if (
        !generatedContent.outcomes ||
        (Array.isArray(generatedContent.outcomes) && generatedContent.outcomes.length < 2)
      ) {
...
function determineComplexity(
  ...
  const allText = [...content.mainConcepts, ...content.keyTakeaways, ...content.bestPractices]
    .join(' ')
    .toLowerCase()
...
function createConceptModule(
  ...
  return {
    ...
    scenario:
      content.useCases[index] ||
      `You need to implement ${concept} monitoring for your application.`,
    codeExample:
...
function calculateCourseDuration(moduleCount: number, complexity: string): string {
  ...
  const totalMinutes = Math.round(
    baseMinutes * (complexityMultiplier[complexity as keyof typeof complexityMultiplier] || 1.0)
  )
...
function generateRelevanceExplanation(
  roleContext: { pain_points: string[] },
  _patterns: ContentPattern
): string {
...
function generateRoleExamples(
  roleContext: { examples: string[] },
  content: SynthesizedContent
): string[] {
