...
class AIGeneratedCoursesStore {
  ...
  getApprovedCourses(): AIGeneratedCourse[] {
    return this.getAllCourses().filter(course => course.approvedAt && course.approvedBy)
  }
  ...
  getApprovalWorkflowByCourse(courseId: string): ApprovalWorkflow | undefined {
    return Array.from(this.approvalWorkflows.values()).find(
      workflow => workflow.course_id === courseId
    )
  }

  getPendingApprovals(): ApprovalWorkflow[] {
    return Array.from(this.approvalWorkflows.values()).filter(
      workflow => workflow.status === 'pending' || workflow.status === 'in-review'
    )
  }
  ...
  getGenerationStats() {
    ...
    const statusCounts = this.getAllCourses().reduce(
      (acc, course) => {
        const progress = this.getGenerationProgress(course.generationRequest.id)
    ...
    const roleDistribution = this.getAllCourses().reduce(
      (acc, course) => {
    ...
    return {
      ...
      averageQualityScore:
        totalCourses > 0
          ? this.getAllCourses().reduce((sum, course) => sum + course.qualityScore, 0) /
            totalCourses
          : 0,
    }
  }
  ...
  getCoursesCreatedAfter(date: Date): AIGeneratedCourse[] {
    return this.getAllCourses().filter(course => course.generatedAt > date)
  }

  getCoursesByQualityThreshold(threshold: number): AIGeneratedCourse[] {
    return this.getAllCourses().filter(course => course.qualityScore >= threshold)
  }
  ...
  importData(data: Record<string, unknown>) {
    ...
    if (data.generationRequests && Array.isArray(data.generationRequests)) {
      this.generationRequests = new Map(
        data.generationRequests as [string, ContentGenerationRequest][]
      )
    }
...
export const getAIGeneratedCourse = (id: string) => aiGeneratedCoursesStore.getCourse(id)

export const getAllAIGeneratedCourses = () => aiGeneratedCoursesStore.getAllCourses()

export const getApprovedAIGeneratedCourses = () => aiGeneratedCoursesStore.getApprovedCourses()

export const mergeCourseData = (existingCourses: Course[]) =>
  aiGeneratedCoursesStore.mergeWithExistingCourses(existingCourses)

export const getGenerationStats = () => aiGeneratedCoursesStore.getGenerationStats()
...
export const getAIGenerationSettings = () => aiGeneratedCoursesStore.getSettings()
