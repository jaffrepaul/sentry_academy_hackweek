...
const LoadingCard: React.FC<LoadingCardProps> = ({ className = '', lines = 3 }) => {
  const cardClasses =
    'backdrop-blur-sm border rounded-2xl p-6 bg-white/75 border-purple-400/40 dark:bg-slate-900/40 dark:border-purple-500/30'
  ...
  return (
    <div className={`${cardClasses} ${className}`} aria-label="Loading content">
      ...
      {Array.from({ length: lines }).map((_, index) => (
        <div
          key={index}
          className={`h-3 ${skeletonClasses} mb-2 ${index === lines - 1 ? 'w-1/2' : 'w-full'}`}
        />
      ))}
