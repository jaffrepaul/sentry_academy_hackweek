  ...
  switch (nodeEnv) {
    case 'production':
      return envSchema.extend({
        ...
        NEXTAUTH_SECRET: z
          .string()
          .min(64, 'Production NEXTAUTH_SECRET should be at least 64 characters'),
...
export function isFeatureEnabled(
  feature: keyof Pick<
    Environment,
    'ENABLE_AI_FEATURES' | 'ENABLE_EXPERIMENTAL_FEATURES' | 'BACKUP_ENABLED' | 'RATE_LIMIT_ENABLED'
  >
): boolean {
  return env[feature]
}
...
export function printEnvConfig() {
  ...
  console.log(
    `   DATABASE_URL: ${env.DATABASE_URL ? env.DATABASE_URL.split('@')[0] + '@[REDACTED]' : 'NOT_SET'}`
  )
